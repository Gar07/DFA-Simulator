...
export default function AutomataNode({ data }: AutomataNodeProps) {
  return (
    <div className={clsx(
      "relative flex items-center justify-center w-16 h-16 rounded-full bg-slate-900 shadow-lg transition-all duration-300",
      data.isAccept ? "border-4 border-double" : "border-2",
      data.isActive ? "border-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.6)]" : "border-slate-500"
    )}>
      ...
      {data.isInitial && (
        <div className="absolute -left-6 top-1/2 -translate-y-1/2 flex items-center text-slate-500">
      ...
      <span className={clsx(
        "font-mono font-bold",
        data.isActive ? "text-cyan-400" : "text-slate-200"
      )}>
        {data.label}
      </span>
